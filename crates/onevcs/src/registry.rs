...
pub struct Registry {
    ...
    ///
    /// Optional, and **omitted when it is absent**, so a version 5 document that
    /// named no such file migrates into a version 6 one that names none either
    /// rather than gaining a null nobody wrote.
    #[serde(default, skip_serializing_if = "Option::is_none")]
