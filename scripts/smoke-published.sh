...
for command in register repos resolve session publish publish-branch change recover recoverable status import integrate sync sweep events artifact rules release; do
  case "$help" in
    *"$command"*) ;;
