...
Patches: 165. Tests observed red and then green: 188.
...
- RED `copies_of_one_branch_that_have_diverged_refuse_the_landing_and_name_each_one` — the copy in <tmp>/.onevcs/workspaces/-tmp-.tmpO3oxCN-project-1fc3dadb99bb/runs/<token>/clone stands on its own parent 5deda8a628fce5b93bdf80
...
- RED `a_copy_amended_in_one_checkout_is_refused_naming_both_trees_and_how_they_differ` — every fact about a copy is named against the checkout it came out of; this one is not:
- RED `copies_of_one_branch_that_have_diverged_refuse_the_landing_and_name_each_one` — the copy in <tmp>/.onevcs/workspaces/-tmp-.tmpcox6s4-project-39616f5cf73b/runs/<token>/clone stands on its own parent 4409d7912bcba63cc48b0c
...
### `140-the-landing-a-merge-recorded-is-never-read`
...
### `141-the-change-requests-number-is-not-looked-for`
...
### `142-a-landing-leaves-no-record-on-the-base`
...
### `143-a-landing-nothing-records-is-read-as-unpublished`

the comparison of content answers no rather than unknown, so a landing nothing recorded reads as work nobody published.

- RED `a_branch_landed_with_no_change_request_and_not_through_this_host_reads_as_unknown` — assertion `left == right` failed: nothing records that it landed, and the base carries what it changed: {"version":2,"ref":{"given":"feature

### `144-a-landed-row-is-offered-to-be-resumed-anyway`

a row whose work reached the base carries the argv that publishes it again, and prints it as ready to resume.

- RED `a_landing_this_host_kept_no_record_of_is_read_off_the_trailer_it_left_on_the_base` — assertion `left == right` failed: a landed row carries no argv, in the document as in the rendering: {"identity":"<tmp>/project","branch":{"
...
### `145-a-branch-that-landed-is-listed-as-work-to-publish`
...
### `146-the-tiers-travel-under-names-nothing-teaches`
...
### `147-work-nobody-published-is-read-as-maybe-landed`
