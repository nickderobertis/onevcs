...
| a broken slot whose clone retains a branch | never recreated; it stands broken and reported, and the open moves on to the next candidate | Recreating a slot is removing its clone, and a branch only that clone carries is the one thing a slot is kept for. A slot with no readable record is judged against the requester's checkout at a placement and the publication checkout at a prune, so one whose clone holds nothing those checkouts do not reach is still recreated or pruned rather than standing broken for ever. |
