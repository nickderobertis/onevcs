...
| Item | Inferred shape | Why |
| --- | --- | --- |
| the registry's `releases` key | optional, at **version 5**, rather than the reason for a version 6 | The plan asked for the version to rise by one. It is shared host state — one document per machine, rewritten in place by whichever `onevcs` migrates it first — so a version an already-released build cannot read stops every verb on a host whose operator configured nothing, and this repository has watched that happen: a suite run wrote a bumped registry into `~/.onevcs` and every `onevcs` command on the host refused until it was restored by hand. The additive key costs an older build nothing when it is absent and is refused by name through `deny_unknown_fields` when it is present, which is the fail-closed property the bump was for. Confirming this means one amendment striking "the version rises by one", not a new shape to approve. |
