...
| --- | --- | --- |
| `Identity` | `origin`, `gate` | The contract says the registry is "v5 = ai-orchestrator's v4 identities/checkouts + rules reference", so the identity record began as v4's, field for field. Two of those fields — `workflow` and `repo_type` — are withdrawn: both were inferred at registration from whether the origin had a host, settable by nothing afterwards, and consulted where the resolved publication policy should have been (the merge train's gate, the merge-path coverage verdict, and the handoff naming the verb for a complete branch). Every one of those reads the rules file's `publication` now, and the review requirement is its `approvals`. Their removal is registry version **6**, one past the `v5` the contract names: a document at 2–5 is migrated on its first read, dropping exactly those two keys from each identity and keeping every other key beyond the shape, so an old host follows its configured policy and stops carrying the inference. The identity *key* is not a field: it is the normalized origin, which is the map key in the document. |
| `Checkout` | `path`, `identity` | v4's checkout record. |
| `Registry` | `version`, `identities`, `checkouts`, `rules` | v4's document plus the rules reference the contract adds, written at version 6 since the two inferred identity fields were withdrawn (the `Identity` row above). `rules` is optional: absent means the built-in default policy. |
...
Version 7 is the held draft: `publication.draft` gains the reason's `kind`
(`awaiting-release` or `held`), because a draft is held for one of two reasons now
and a consumer routes on which; `publication.held_as_draft` is what the *host* says
stands, where it could be asked, beside what this host's record says holds it; and
`publication.described` is the last `change-described` on the stream — when, the
...
| `publication.draft` | the `DraftReason` currently holding the change back, omitted where nothing is | The one thing a draft change request cannot tell anybody. The approved amendment puts the reason in the publication record — the session's own event stream — and **nothing in the change request's body**, so a host renders that the change *is* a draft and never why. This report is where that record is read back, because it is the only place in this crate that reads a stream for a person. The reason is `DraftReason` itself rather than a second shape beside it, `kind` and all: a consumer that routes on `awaiting`/`target`/`reference` to decide when to lift is reading the same fields the publication was given, and one that reads `held` knows the session holding it is what lifts it. Omitted for a change nobody drafted **and** for one whose draft has been lifted, which are one answer — nothing is holding this back now — and the history of both is in `onevcs events`. |
| `publication.held_as_draft` | what the host says, omitted where it could not be asked or would not say | The record above says why a draft was opened; the host says whether one stands, and the two are two facts — a draft lifted on the host by hand is one the record still holds a reason for. Never `false` for a host that would not say: "not a draft" is what lets a change be asked to merge. |
| `publication.described` | the newest `change-described`, omitted where there is none | `change describe` is the one write to a change request's prose after it exists, and the stream is the only record of it. The report names when, the artifact the body is readable under, and the title where the description replaced it — never the body itself, which is prose of unbounded size and is what `onevcs artifact cat` is for. |
...
| tier 4 | the content comparison, over the paths the branch touched, and never a `yes` | Last, and labelled as the inference it is. Scoped to the paths the branch touched rather than the whole tree, so unrelated work landing beside it does not change the answer. The base carrying what the branch changed is `unknown` — consistent with a landing nobody recorded, and with somebody else making the same change. **Kept, and deliberately.** Removing it was the second of two fixes proposed for the defect `in-part` closes, and it is the wrong one: its `no` is the only answer available for a branch with no session record, no change request and no trailer — one cut by hand, or one whose records a host has lost — and there are dozens of those on a working host. Without the tier every one of them reads `may have landed`, which is the answer that hides preserved work. The first fix trades nothing; the second trades a wrong `no` about landed work for a wrong `unknown` about unlanded work, and the second is worse. |
...
| the guard on tiers 1–3 | the landing commit is asked whether it carries everything the branch changed since it forked; a branch holding anything it did not is `in-part`, answered by the tier that found the landing | A landing lands what the branch carried *then*, and a session continuing a name that already means something commits onto the same branch — a row that read that as finished would hide unpublished work, the one direction this must never fail in. Asked of the *landing* commit rather than of the base as it stands now, which is what keeps the guard from being the inference the tiers replace: the base moves, and the commit that landed the work does not. What the guard decides is *which* of a tier's two answers it gives, and no longer whether that tier answers at all: declining used to drop the finding and fall through, so the strongest evidence there is — a merged change request whose URL the same report prints two lines above — was discarded for a comparison that cannot survive a squash merge, and a retried branch is the ordinary case. A lower tier is still asked first, because a branch that landed twice has a second landing that does account for all of it; the kept finding answers only once none has. |
| `Landed` | `yes` (carrying its evidence) / `in-part` (its evidence and `unlanded`) / `no` / `unknown` | Four answers because each is real. The third and fourth: a branch that landed with no change request and not through this crate leaves nothing in history to read, and reporting that as `no` is what puts a resume instruction under work that is already on the base. `in-part` is the state the code had no word for — *a known landing with unlanded commits above it* — and it is what a retried dispatch leaves behind, which on a host that retries is the normal branch. The evidence travels *inside* both answers a record decided, so a landing with nothing behind it is unrepresentable. |
| `in-part`, on each of the two questions asked of a landing | **not** a landing for "is there work left to publish"; **is** a landing for "what commit did this work reach the base at" | The two readers want opposite halves of one answer, which is why one alternative carries both rather than two answers sitting beside each other. `is_landed()` is the first question and answers `false`, so `recoverable` keeps the row and its `recover_command`, `Landing` is not `landed`, and nothing tells an operator the work is finished. `release status`, `release acknowledge` and the release correlation on a session's stream ask the second and take `evidence.commit()`, because the part that landed reached the base at exactly that commit — which is what lets a hold on a release that has already happened clear instead of waiting for ever. |
| `unlanded` | how many commits the branch holds that the landing does not integrate, as a `NonZeroUsize` | The count is what makes the answer readable as an amount of work: one commit above a landing is a follow-up, thirty is a branch whose landing says almost nothing about it. Counted by walking the branch newest-first and stopping at the first commit the landing already integrates — a squash lands a prefix, so what is left over is a suffix — which costs one question per commit it counts rather than one per commit the branch has. The tip is counted rather than asked about, because the guard has just been told the landing does not integrate it, which is what makes the count non-zero *by construction* rather than by a check. Zero is not a value it can hold: a landing with nothing above it is `yes`, and a document claiming an in-part landing of nothing is refused where it is read. |
...
| `publication.state` | the seven it had, plus `maybe-landed` and `landed-in-part` | The eighth is the answer version 1 had no room for and reported as `landed`; the ninth is version 5's, and it is `Landed::InPart`'s word. Every one of these is **derived** from `landed` rather than written beside it, which is what stops the two from disagreeing — a report saying `landed-in-part` beside an answer that is not `in-part` is refused where a report is read. The human rendering's `landed:` line is the word it always was (`in part`, here) with the tier that decided it, and the count the landing does not carry, on the line below. |
...
| `recoverable --session` / `--label` | optional and repeatable; the branches the named sessions hold or held, and the branches of the sessions carrying every label pair given — combinable with each other and with `--repo` | The join a consumer used to make by hand: which of these preserved branches are *this run's*. It is one filter with one meaning — a row is answered with when some session record of this host naming its branch was selected — so a branch a retried session continued is still that session's, which is what "hold or held" means and what filtering on the row's own `session` field could not have said. A token no record names is refused **by name**: "nothing of that session is left to publish" and "there is no such session" are different answers, and a consumer holding its work behind the first must never be handed it in place of the second. A label pair nothing carries is the first of those and answers an empty report with status `0`. Both narrow the *scan* rather than the output — an identity none of the selected sessions belongs to is never opened, a checkout none of them can hold a branch in is never opened, and a name none of them holds is never counted — which is the whole point: a hook asking this question at the end of every turn cannot afford a read that walks the host to answer about three branches. Every rendering names the narrowing, for the reason `--repo` and the withheld note are named: an answer narrowed to one run's sessions reads exactly like an empty host. |
| `Recoverable.session` / `.labels` | the token of the newest session record naming the row's branch — `null` for a branch no record names — and that session's labels, `{}` where there are none | Written on every row, `null` included, so a reader routing on them meets the key everywhere. The newest record is the end of the branch's chain of retries, an open one preferred, which is the preference `onevcs status` already makes when it picks whose evidence a branch's is — so the two cannot come to name different sessions for one branch. A `worktree-agent-*` orphan is what `null` is for: work left behind by something this crate never opened a session for. |
...
| what "no origin has" means | commits no `origin` ref carries **and** nothing decides that its work reached the base | `vcs::collect`'s own two questions, so the report that offers work for recovery and the rule that keeps its workspace cannot disagree. Publication squashes: ancestry alone would call every finished workspace unpublished work. The second question is `landed::decide`'s and is asked through it, which is the correction: it used to be a bare comparison of trees, which is that module's *last* tier and the one it says must never answer `yes` — so a workspace whose clone had not fetched since before its branch landed was kept for ever on a comparison against a base predating the evidence, while the listing beside it dropped the branch as landed. A workspace whose branch is decided landed is reclaimable; one decided landed **in part** is retained, because the commits above the landing are work nobody published; `no` and undecidable retain, the second most of all, since a workspace that may hold the only copy of somebody's work must never go on an answer nothing could decide. |
...
| how an `InstructionTemplate` is read back out | `Deref<Target = str>`, `Display`, and `Into<String>` — the accessors `Prose` already has, and no named one beside them | The approved contract declares the newtype and its three checks and stops there. A consumer has to get the template out to hand it to its own engine, and the cheapest honest answer is the shape the newtype next door already uses, rather than a `source()` this crate would be inventing. |
| a template whose *variables* this crate does not recognize | accepted | The approved contract fixes nine variables and they are the **consumer's**: a plan node id, a branch, a landing commit — none of which onevcs has. A build refusing a name it did not know would refuse variables a consumer genuinely supplies, so the conversion checks that a template *parses* and stops. |
| a `Declaration` a caller built at a version below the one that declares `adoption_instructions` | refused where it is validated, naming the target and both versions | A *document* in that state is refused by name before it deserializes, so this covers only the caller who built the value. Without it `render_release_declaration` would write a file its own reader refuses by name — which is the one thing rendering promises not to do, and the promise is stated in the contract rather than inferred. |
| the host document's key for a host target's own template | `adoption_instructions`, the same spelling the declaration uses, with no `releases.yml` version bump | The approved contract says `ReleaseTarget` gains the field and does not spell the YAML key; one spelling for one concept is what stops a host writing the producer's name and getting nothing. No bump because `releases.rs` declares no `deny_unknown_fields` and the module already states that a document a newer build wrote loads on an older one. |
...
### Proposals not built, because the approved contract is silent on them

Root cause 18 part D was approved as proposed. Three things an implementation reaches
for are **not** in it, and are recorded here rather than added — a shared surface six
repositories write against is not one to widen unilaterally.

- **A renderer on this crate's surface.** The approved contract puts one function in the
  *consumer*, "so a template cannot render differently in the two places", and fixes the
  nine variables it renders against — all of them the consumer's. A `render_release_*`
  here would be a second renderer with a smaller variable set, which is the thing that
  sentence forbids. If a onevcs-side render is ever wanted, it needs the variable set
  settled first, and that is a contract question rather than an implementation one.
- **A public constant for the name `producer`.** The contract fixes the *string* — a
  host template begins `{% extends "producer" %}` — and registration happens wherever the
  rendering does, which is not here. A `pub const` would be this crate exporting a name
  it never uses.
- **Refusing a template that is non-empty but blank.** The conversion is specified as
  "non-empty, at most 4000 bytes, and it parses as a minijinja template", so `"   "` is
  accepted. Refusing whitespace-only would be a fourth check, and every other blank-vs-
  empty rule in this document was decided at the contract rather than at the boundary.
...
| a broken slot in `WorkspaceCapacity` | counted in `slots`, in none of `idle`, `in_use` and `maintaining`, and in `admits` and `admitted` | The declared struct has no field for a count of broken ones, and `idle` has to mean idle. A broken slot is still takeable — the session that takes it recreates it in place — so the two answers about admission count it; `pool status` reports each slot's state for a reader who needs the rest. |
...
| what recreating a broken slot keeps | its clone, whenever the clone is still a repository, with every ref it retains: an unreadable record is rewritten beside it, a lost worktree is cut again from it; only an unusable clone is removed | The amendment says recreated in place, and a slot's clone is where a branch the hand-back could not copy lives, so recreation rebuilds what is broken and no more. A slot whose record is unreadable is rebound to the checkout its clone borrows objects from — read off `objects/info/alternates` — or to the requester where that cannot be read, since the alternates are what the clone actually reads through. A slot with no readable record is judged against the requester's checkout at a placement and the publication checkout at a prune, so one whose clone holds nothing those checkouts do not reach is still pruned rather than standing for ever. |
| what `pool prune` removes | every slot nobody is in — idle, or broken — whose clone retains no branch; a broken slot whose clone cannot be asked is kept, naming that | The amendment says idle. A broken slot is takeable the way an idle one is — the next open recreates it in place, keeping its clone and every ref it retains — so a prune that kept a broken slot retaining nothing would keep exactly what the next open would rebuild from nothing, and the same retained-branch test decides both. |
...
| a `delete` entry's shape | a relative path with no `..` component, refused at load; an entry that is itself a symlink is unlinked rather than followed, and one reached *through* a symlink is refused at the return, naming the link | The amendment says relative and inside the worktree, and the document can only be held to that lexically: a link on the way to the entry could carry the removal anywhere, so the filesystem is held to it where the deletion happens. Only an ignored link can be on the way, since an untracked one is committed with the rest of a dirty tree before anything is deleted. |
