...
and means "no release targets" — the key is absent, which is what that is spelled
as — and it is migrated lazily on the first read that needs it, exactly as a v2–v4
document is: nothing is filled in, only the number moves. An absent key stays
absent through that migration rather than becoming a null nobody wrote. A host with
no release-targets document at all behaves exactly as it did before there was one:
